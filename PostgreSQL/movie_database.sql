...
/*
1. To select all the data from a table(rows and column)
...
/*
2. To select one column  from a table( 1 column)
...
/*
3. To select multiple columns from a table( more than 1 column)
...
/*
4. To select columns from a table alongside with filter using WHERE clause 
...
/*
5. To select columns from a table alongside with filter using AND with WHERE clause 

Using AND along with WHERE clause, both condition must be satisfied 
*/
...
/*
6. To select columns from a table alongside with filter using OR with WHERE clause 

Using OR along with WHERE clause, only one of the  condition must be satisfied
*/
...
/*
6. To select columns from a table alongside with filter using WHERE clause 
*/
...
-- Logical condition can also be usec for date also 

SELECT * FROM movies
WHERE release_date > '1999-12-31'
AND movie_lang = 'English'
AND age_certificate = 'PG'; 
...
-- TO SELECT A COLUMN THAT IS EQUAL TO MORE THAN ONE VALUE (multiple selection from same column)
/*
...
-- To find first and last name where the first name have either 'Bruce','John','Peter' (IN)

SELECT first_name, last_name FROM actors
WHERE first_name IN ('Bruce','John','Peter');

-- To find all other name in the list aside the stated one (NOT IN)
-- WILL return all other name expect those three name in the first name. 
...
/*
Using IN and NOT IN with integer
*/

SELECT actor_id, first_name, last_name FROM actors
WHERE actor_id IN (2,3,4,5,6,8);  -- return actors with the following actor_id

SELECT actor_id, first_name, last_name FROM actors
WHERE actor_id NOT IN (2,3,4,5,6,8); --return actors not include in this actor_id
...
-- using % implies any characters(one or more than one)

SELECT * FROM actors
WHERE first_name LIKE 'P%'  -- first name starting from P

-- using '_' implies any character(one )

SELECT * FROM actors
WHERE first_name LIKE 'Pe_'  -- first name starting from P

SELECT * FROM actors
WHERE first_name LIKE '%r'  -- first name end with r

SELECT * FROM actors
WHERE first_name LIKE '__rl__'  -- first name with rl in middle and any two character at the start and end
...
--Q1
SELECT movie_name, movie_lang FROM movies
WHERE movie_lang IN ('English','Spanish','Korean');    --Ans: 36 movie_name
...
AND date_of_birth BETWEEN '1940-01-01' AND '1969-12-31';  -- Ans: 6 name 

--Q3
SELECT first_name, last_name FROM directors
WHERE nationality IN ('British','French','German')
AND date_of_birth BETWEEN '1950-01-01' AND '1980-12-31';    --Ans: 4 name


--ORDER BY :Use for ordering returned result
...
ORDER BY first_name DESC;    -- Ordering from A - Z

SELECT first_name,last_name, date_of_birth FROM actors
ORDER BY first_name ASC;  --orering from A - Z

SELECT actor_id,first_name,last_name, date_of_birth FROM actors
ORDER BY actor_id DESC;  --order by integer 

--Using WHERE clause alonside ORDER BY
SELECT actor_id,first_name,last_name, date_of_birth FROM actors
WHERE gender = 'F'
ORDER BY date_of_birth DESC;


-- LIMITING THE NUMBER OF RECORDED RETURNED
...
-- OFFSET: ignorn the nunber of row you set it to. 
SELECT * FROM movie_revenues
ORDER BY revenue_id
LIMIT 10 OFFSET 2;


--FETCH : It can alos be used to get a certain amout of number(just like offset)
/*
...
FETCH FIRST 1 ROW ONLY;    -- Used to select the first row 



SELECT movie_id, movie_name FROM movies
FETCH FIRST 10 ROW ONLY;    -- Used to select the first 10 rows

-- Usinf OFFSEt alongside with FETCH
...
-- selecting more than one distinct attribute
